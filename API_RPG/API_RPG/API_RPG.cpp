...
HBITMAP character;
Vector2 characterRect(32,40);

HBITMAP characterMask;
...
)
{
	...
	case WM_CREATE:								// 윈도우가 생성될때	
		...
		character = LoadBitmap(hINSTANCE, MAKEINTRESOURCE(Character));
		character = (HBITMAP)LoadImage(NULL, TEXT("Character.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);

		characterMask = (HBITMAP)LoadImage(NULL, TEXT("CharacterMask.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	...
	case WM_PAINT:	//윈도우 사이즈 변경 및 최소화 최대화가 발생될때 호출되는 메시지
		...
		CreateRect(hMyBrush, hOldBrush, hDCBuffer, hWnd, RGB(255, 255, 255), -1, -1, window.x, window.y);

		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				DrawBitmap(hDCBuffer, i * characterRect.x, j * characterRect.y, characterRect.x, characterRect.y, i * characterRect.x, j * characterRect.y, character,characterMask);


		//CreateRect(hMyBrush, hOldBrush, hDCBuffer, hWnd, RGB(0, 0, 0), 56, 30, 30, 30);
